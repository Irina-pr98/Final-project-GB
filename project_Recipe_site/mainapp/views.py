    ...
    if request.method == 'POST':
        ...
        if recipe_form.is_valid():
            new_recipe = Recipe(
                ...
                portions=recipe_form.cleaned_data['portions'],
            )
...
def read_recipe(request, pk: int):
    ...
    context = {
        ...
        'ingredients_list': recipe.ingredients.split('\r'),
        'cooking_time_list': recipe.cooking_time.split('\r'),
        'portions_list': recipe.portions.split('\r'),
        'calories_list': recipe.calories.split('\r'),
        'recipe': recipe,
...
def update_recipe(request, pk: int):
    ...
    data_for_form = {
        ...
        'ingredients': update_item.ingredients,
        'cooking_steps': update_item.cooking_steps,
        'cooking_time': update_item.cooking_time,
        'img': update_item.img,
        'author': request.user,
        'calories': update_item.calories,
        'portions': update_item.portions,
    ...
    if request.method == 'POST':
        ...
        if CreateRacipeForm(request.POST, request.FILES).is_valid():
            ...
            update_item.ingredients = request.POST.get('ingredients')
            ...
            update_item.calories = request.POST.get('calories')
            update_item.portions = request.POST.get('portions')
